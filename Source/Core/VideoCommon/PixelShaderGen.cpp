                       ...
                       APIType api_type, bool stereo)
{
  ...
  if (stage.tevorders_enable)
  {
    ...
    out.Write("\ttextemp = ");
    if (stage.epsilon_hack)
    {
      SampleTexture(out, "(float2(tevcoord.xy) + 0.25)", texswap, stage.tevorders_texmap, stereo,
                    api_type);
    }
    else
    {
      SampleTexture(out, "float2(tevcoord.xy)", texswap, stage.tevorders_texmap, stereo,
                    api_type);
    }
  }
  else
